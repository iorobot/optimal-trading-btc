...
class Plot:
    ...
    def draw(df, df_trades, pair, strategy_info):
        ...
        #local_tz = get_localzone()                                                         - not necessary with latest version of pandas
        # Convert datetime to current time-zone
        #df['date'] = df['date'].dt.tz_convert(local_tz)                                    - not necessary with latest version of pandas
        df = df.set_index(['date'])
        df_index = df.index.tz_localize(None)

        #df_trades['date'] = df_trades['date'].dt.tz_convert(local_tz)                      - not necessary with latest version of pandas
